...
## Strict Execution Guardrails (Context & Token Economy)
Чтобы не исчерпать контекстное окно и лимиты API, ОБЯЗАТЕЛЬНО соблюдай следующие правила при выполнении любых задач:

1. **Запрет на слепой поиск (No Broad Scans):**
   - НЕ запускай широкомасштабный поиск (`grep`, `find`, `glob`) по всему репозиторию без явного указания пользователя.
   - Читай ТОЛЬКО те файлы, которые явно указаны в промпте задачи или напрямую импортируются целевым модулем.
...
3. **Локальный контекст:**
   ...
   - Не выполняй `git commit` и `git push`, если пользователь не попросил об этом явно. Показывай результат в виде `git diff`.
