...
ANTI_SYCOPHANCY_INVARIANTS = (
    ...
    "  On PASSED_WITH_GLOSS specifically: the sub-concept is credited at the "
    "REQUIRED threshold only. When its own [W/H/M] flags in "
    "[CURRENT_CONCEPT_MAP] are not all 1 (e.g. MECHANIC still 0, offered only "
    "as an optional self-gloss), `confirmation` is FORBIDDEN from: (a) any "
    "full/complete claim about the sub-concept itself — «полностью/целиком "
    "освоено/засчитано», «100%»; (b) crediting the USER with having worked "
    "out, analyzed, or gotten right the content of the still-open layer — "
    "e.g. «механика ... разобрана верно», «механизм ... разобран верно», "
    "«[MECHANIC-слой] понят/усвоен». You (the tutor) may gloss that layer "
    "yourself in `technical_explanation`, but `confirmation` must credit only "
    "the layers the user actually cleared (WHY/HOW), never attribute the open "
    "layer's correctness to the user — that mismatch reads as contradicting "
...
EVALUATOR_SKIPPED_TUTOR_RULES = (
    ...
    "(control chip, lecture request, rephrase/clarify request, empty/short "
    "text, or no pending evaluation target).\n"
    "Output DeepDiveExplainContract: technical_explanation + optional "
    "follow_up_question only. Teach or clarify; do not grade.\n"
    "If current_user_message is asking you to rephrase/restate the pending "
    "question in different words (not a lecture request) — the question is "
    "still open and NOT answered yet: restate the SAME technical question in "
    "`follow_up_question` using clearer/different wording, still targeting the "
    "same `question_sub_concept_id`. Do not invent a new question, do not "
    "switch sub-concept, do not summarize [CURRENT_CONCEPT_MAP] as passed or "
    "failed.\n"
)
